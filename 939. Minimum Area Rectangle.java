...
class Solution {
    public int minAreaRect(int[][] points) {
        Map<Integer, Set<Integer>> xToY = new HashMap<>();// x to all the y 
        for (int[] p : points) {
        ...
        for (int i = 0; i < points.length; i++) {
            for (int j = i + 1; j < points.length; j++) {
                ...
                if (!xToY.get(points[i][0]).contains(points[j][1])|| !xToY.get(points[j][0]).contains(points[i][1])) {
                    continue;
                }
                int cur = Math.abs(points[i][1] - points[j][1]) * Math.abs(points[i][0] - points[j][0]);
